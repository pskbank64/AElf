...
namespace AElf.Synchronization.BlockExecution
{
    public class BlockExecutor : IBlockExecutor
    {
        ...
        private void InsertTxs(List<TransactionResult> txResults, IBlock block)
        {
            ...
            stopwatch.Stop();
            _logger.Info($"Performance-[DB-InsertTxs]: transaction count: [{txResults.Count}], spent time: [{stopwatch.ElapsedMilliseconds}ms]");
        }
        ...
        private async Task TxRefBlockValidatorUpdateCrossChainInfo(IBlock block, List<TransactionResult> txnRes)
        {
            ...
            stopwatch.Stop();
            _logger?.Info($"Performance-[UpdateCrossChainInfo]: Block index: [{block.Index}], spent time: [{stopwatch.ElapsedMilliseconds}ms]");
